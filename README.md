## 👋 안녕하세요 이정민의 깃허브 페이지 입니다
컴퓨터공학과를 졸업한 후 부트캠프에서 다양한 기술 스택을 학습하였습니다. 이에 더불어 여러 IT 자격증을 통한 전공 지식을 바탕으로 IT인프라의 메커니즘을 이해하고, IT서비스데스크에서 근무하며 전산 직무 역량을 키우고 있습니다.

## 🏫 History
| 이름                     | 기간                 |
|------------------------|--------------------|
| 성남고등학교 졸업 | 2016.03 ~ 2019.01  |
| 성결대학교 컴퓨터공학과 졸업 (3.8) | 2019.03 ~ 2025.02  |
| 멋쟁이사자처럼 자바 스프링 백엔드 15기 | 2025.02 ~ 2025.06  |
| 캡스텍 IT서비스데스크 (카카오모빌리티 파견직) | 2025.06 ~ 재직중 |

## 📜 Certification
- `2011.03` DIAT 프리젠테이션 고급
- `2022.02` 컴퓨터활용능력 1급
- `2022.09` 리눅스 마스터 2급
- `2024.12` 정보처리기사
- `2024.12` SQL 개발자
- `2025.09` 데이터 분석 준전문가
- `2025.09` 네트워크관리사 2급

## 💻 Project
- 👤 [카카오모빌리티 IT서비스데스크 방문자 기록 시스템](https://github.com/zshfz/html-js-firebase-servicedesk)
- ✏️ [아지트 안내글 템플릿 시스템](https://github.com/zshfz/html-js-manifestv3-agittemplate)
- 👑 [실시간 채팅 기반 게임 프로그램](https://github.com/zshfz/react-nodejs-socketio-grabthecrown)
- 📝 [CRUD 웹사이트](https://github.com/zshfz/spring-mysql-website)

## 📝 Portfolio
[![NOTION](<img src="https://img.shields.io/badge/Notion-000000?style=for-the-badge&logo=Notion&logoColor=white">)


